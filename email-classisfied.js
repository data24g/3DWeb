...
function checkSpam(title, content, from_email) {
    ...
    const spamConfidence = Math.min(matchCount * 0.3, 1);
...
function checkSuspicious(title, content, from_email) {
    ...
    const suspiciousConfidence = Math.min(matchCount * 0.35, 1);
...
function checkSafe(title, content, from_email) {
    ...
    const safeConfidence = isSafe ? Math.min(safeScore * 0.25, 1) : 0;
